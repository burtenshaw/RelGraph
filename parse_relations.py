...
def parse_books(books_dir, do_simple = False, do_ling = False, do_chunks = False):
    ...
    for p in paths:
        ...
        _df = pd.DataFrame(parse_paragraphs(soup))
        _df['file_name'] = p
        _dfs.append(_df)
...
with pd.ExcelWriter(os.path.join(data_dir, 'relations.xlsx')) as writer:
    for book in books:
...
if __name__ == "__main__":
    ...
    df.to_json(os.path.join(data_dir,'DATA_1_2_21.json'))
    cdf.to_json(os.path.join(data_dir,'CHARACTERS_1_2_21.json'))

    # df.to_pickle(os.path.join(data_dir,'DATA_1_2_21.bin'))
    # cdf.to_pickle(os.path.join(data_dir,'CHARACTERS_1_2_21.bin'))